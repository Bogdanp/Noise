...
(provide
 current-write-backend-code?
 write-Swift-code)

(define current-write-backend-code?
  (make-parameter #t))
...
(define (write-backend-code out)
  (fprintf out "public final class Backend: Sendable {~n")
  (fprintf out "  let impl: NoiseBackend.Backend!~n~n")
  (fprintf out "  init(withZo zo: URL, andMod mod: String, andProc proc: String, andBootArguments ba: BootArguments = .init()) {~n")
  (fprintf out "    impl = NoiseBackend.Backend(withZo: zo, andMod: mod, andProc: proc, andBootArguments: ba)~n")
  (fprintf out "  }~n")
